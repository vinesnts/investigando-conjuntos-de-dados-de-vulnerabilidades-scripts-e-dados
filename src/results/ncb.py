...
for project in projects:
  ...
  print(f"Média: {np.mean(np.array(ncb))}")
  print(f"Mediana: {np.median(np.array(ncb))}")
  print(f"Máx.: {np.max(np.array(ncb))}")
  print(f"Mín.: {np.min(np.array(ncb))}")
  print(f"Média aparada.: {trim_mean(np.array(ncb), 0.1)}")
